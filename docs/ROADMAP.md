...
สถานะ product shell เมื่อ 2026-08-26: `/app` เปลี่ยนจาก Safety Skeleton เป็น dashboard ตามบทบาทที่นำด้วย farm operations/data analytics และวาง research provenance เป็นส่วนรอง พร้อมเชื่อมต่อกับ farm core และ cash ledger จริงสำหรับเกษตรกร โดยแสดงยอดเงินสดและกำไร/ขาดทุนจริงเมื่อมีข้อมูล และคงสถานะ `empty` หรือ `not_enabled` อย่างซื่อสัตย์เมื่อไม่มีข้อมูลหรือสำหรับบทบาทอื่น

สถานะ increment 5 เมื่อ 2026-08-26: vertical slice **Farm Core + Cash Ledger Foundation** เสร็จสมบูรณ์ตั้งแต่ migration `202608260005` (farmer, farm, plot, expense, sale, 16 secure RPCs, audit allowlist, hard-delete rejection triggers) → domain models & canonical decimal arithmetic → server gateways & actions → production routes `/app/gardens` และ `/app/garden-account` → dashboard overview integration → pgTAP (69 subtests, 300 total assertions) → Vitest (318/318 tests) → Next.js production build → Playwright local E2E (24/24 tests) Reconciliation fixture คืนค่า cash profit เท่ากับ 9,000.25 ตามข้อกำหนดอย่างแม่นยำ
