...
สถานะ increment 5 เมื่อ 2026-08-26: vertical slice **Farm Core + Cash Ledger Foundation** ทำ farmer-owned `farmer`, `farm`, `plot`, `expense`, `sale`, 16 RPCs, production routes `/app/gardens`/`/app/garden-account` และ dashboard cash aggregate แล้ว พร้อม exact decimal boundary, PII-safe audit, plot-area invariant, active-only report และ rollback rehearsal Reconciliation fixture คืนค่า cash profit `9,000.25` ส่วน `activity`, `harvest`, optional `sale.harvest_id`, assigned-collector baseline, hosted migration และ real-data gates ยังไม่เสร็จ จึงนับ increment 5 เป็น **partial** ไม่ใช่ complete
