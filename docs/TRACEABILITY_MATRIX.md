...
| FR-03 | reproducible active sample | research_manager | sampling draft/lock/activate/cancel | sampling_run, sample_member | UNIT-01, UNIT-02, UNIT-08, INT-02, E2E-03, AUD-01 | N/e/n, allocation, NFC/seed/digest/algorithm, candidate hash, ordered result plus `ordered-result-sha256-v1` hash, lifecycle audit |
...
| NFR-04 | sampling reproducibility | research_manager, evaluator_readonly | replay/evidence | sampling_run, sample_member | UNIT-01, UNIT-02, UNIT-08, INT-02, AUD-01 | identical candidate hash/member order/allocation and ordered-result hash from algorithm evidence |
...
| NFR-04 | persisted sampling receipt includes seed, candidate/result digest and ordered stratum evidence for the active run | focused E2E confirms exact `N`, `e`, `n`, seed, algorithm, digest and active status after reload; deterministic unit/database evidence remains separately bounded |
...
| NFR-06 | manager confirmation dialogs receive focus, the primary journey completes, both configured viewports have no document/table horizontal overflow, and the unfocused skip link is hidden after heading focus | focused local assertions pass; no axe scan was added to this sampling-only run |
