...
| NFR-06 | manager confirmation dialogs receive focus, the primary journey completes, both configured viewports have no document/table horizontal overflow, and the skip link remains keyboard reachable while unfocused off-canvas | focused local assertions pass for Tab→Enter focus transfer to `main` and blurred evidence captures; no axe scan was added to this sampling-only run |

หลักฐานนี้เป็น local synthetic acceptance evidence ของ sampling slice โดย milestone verification ผ่าน lint, typecheck, 250 unit/component tests, production build, database lint, 219 pgTAP assertions และ 22 local-auth E2E tests แล้ว แต่ไม่ใช่ V1 acceptance sign-off และไม่อนุญาตให้สรุป hosted migration หรือ real-data readiness
