...
| `sampling_run` | `workspace_id`, `version`, population FK, N/e/target, `seed_text`, `seed_normalized`, `seed_digest_hex`, `seed_u32`, `algorithm_version`, `ordered_candidate_set_hash`, `status` | status exactly `draft\|locked\|active\|superseded\|cancelled`; identifier `sha256-mulberry32-fy-v1`; immutable after lock; active exactly one/workspace เมื่อเริ่ม sampling |
