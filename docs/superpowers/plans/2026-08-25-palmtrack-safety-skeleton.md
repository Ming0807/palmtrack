...
Install exact compatible versions of the Supabase CLI, `@supabase/ssr`, `@supabase/supabase-js`, and Zod. Define the public URL/publishable-key contract, retain the legacy anon-key name only as a compatibility fallback, and define server-only service-role names without values. The environment parser must distinguish `unconfigured`, `configured`, and `invalid`; tests must prove no secret is exposed to client modules or error text.
...
- `recovery_relink_auth_user(profile_id, new_auth_user_id, reason, recovery_reference)` is out-of-band database recovery only. A controlled database operator must explicitly assume a hardened `NOLOGIN` recovery-executor role after establishing verified active-admin JWT context; `anon`, `authenticated`, `service_role`, and normal application routes receive no execution path. The transaction audits the relink without storing credentials or token material.
...
Audit append-only behavior is database-hard. Revoke direct insert/update/delete/truncate from client roles and route internal writes through a fixed-`search_path` `SECURITY DEFINER` function owned by a dedicated `NOLOGIN` audit-writer role that is never granted to application or service roles. Supabase hosted's non-superuser migration operator retains the provider-created administration grant plus a separate `ADMIN FALSE, INHERIT FALSE, SET TRUE` grant in the internal roles because PostgreSQL requires `SET ROLE` capability for function-owner transfer; the migration rejects every other membership. An always-enabled `BEFORE INSERT` trigger rejects inserts unless execution is inside that writer-owner context; always-enabled mutation triggers reject `UPDATE`, `DELETE`, and `TRUNCATE` regardless of RLS bypass. A separate hardened `NOLOGIN` transaction-owner role is the only explicit writer caller, and only allowlisted transaction functions run as that owner. Actor/workspace are derived from authenticated or verified recovery context, never accepted from browser payload; action codes and detail keys are database-allowlisted for this slice.

Verification: local migration reset plus pgTAP proves anonymous/inactive/unknown-role/cross-workspace denies, exact allowlist behavior, zero-before-bootstrap/one-after-bootstrap/two-denied workspace behavior, immutable ID/workspace/auth linkage, admin configuration field allowlists, admin denial on research/ledger/identity relink, fixed current-profile projection with no `auth_user_id` or cross-profile access, and append-only audit. Audit tests attempt forged actor/workspace, direct insert, update, delete, and truncate as authenticated users, admin, `service_role`, and database owner; the insert guard and immutable-row triggers must reject direct mutation even when RLS is bypassed, while approved transaction functions still append the expected event. If the local Docker/PostgreSQL prerequisite is unavailable, record the tooling blocker and keep SQL status unverified rather than claiming RLS pass.
