  ...
  return (
    <section className={styles.workbench} aria-labelledby="sampling-workbench-title">
      ...
      <div className={styles.worksheet} aria-hidden={modalOpen ? "true" : undefined} inert={modalOpen}>
        {canMutate && acceptedImports.length > 0 && <form className={styles.inputSection} action={submitPreview} onSubmit={capturePreviewInput} aria-describedby="sampling-input-note">
      ...
      {modalOpen && confirmation && <ConfirmationDialog confirmation={confirmation} pending={lifecyclePending} cancelReason={cancelReason} setCancelReason={setCancelReason} submitLock={submitLock} submitActivate={submitActivate} submitCancel={submitCancel} onDismiss={() => setConfirmation(null)} restoreFocusRef={restoreFocusRef} />}
    </section>
  );
}
