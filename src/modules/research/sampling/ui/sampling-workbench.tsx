...
import { canonicalizeMarginOfErrorText, type SamplingEvidence } from "@/modules/research/sampling/domain/deterministic-sampling";
  ...
  return (
    <section className={styles.evidenceSection} aria-labelledby={titleId}>
      ...
      <dl className={styles.formula}>
        ...
        <div><dt>ค่าคลาดเคลื่อน e</dt><dd>{evidence.marginOfErrorText ?? number(evidence.marginOfError)}</dd></div>
...
function snapshotFromForm(formData: FormData): PreviewInputSnapshot {
  const rawMargin = String(formData.get("marginOfErrorText") ?? formData.get("marginOfError") ?? "");
  let margin = rawMargin;
  try { margin = canonicalizeMarginOfErrorText(rawMargin); } catch { /* server returns the safe invalid state */ }
  return { populationImportId: String(formData.get("populationImportId") ?? ""), marginOfError: margin, seedText: String(formData.get("seedText") ?? ""), stratumDefinitionVersion: String(formData.get("stratumDefinitionVersion") ?? STRATUM_DEFINITION_VERSION) };
}
...
function inputErrors(populationImportId: string, acceptedImportCount: number, marginOfError: string, seedText: string): Record<string, string> {
  ...
  try { canonicalizeMarginOfErrorText(marginOfError); } catch { errors.margin = "ระบุค่า e ระหว่าง 0 ถึง 1"; }
    ...
    <section className={styles.workbench} aria-labelledby="sampling-workbench-title">
      ...
      <div className={styles.worksheet} aria-hidden={modalOpen ? "true" : undefined} inert={modalOpen}>
        {canMutate && acceptedImports.length > 0 && <form className={styles.inputSection} action={submitPreview} onSubmit={capturePreviewInput} onInvalid={() => setValidationAttempted(true)} aria-describedby="sampling-input-note">
          ...
          <input type="hidden" name="marginOfError" value={(() => { try { return canonicalizeMarginOfErrorText(marginOfError); } catch { return marginOfError; } })()} />
