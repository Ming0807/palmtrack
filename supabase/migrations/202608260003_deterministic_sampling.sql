...
grant select, insert on table public.sampling_allocation, public.sample_member to palmtrack_transaction_owner;
...
as $$
begin
  if current_setting('palmtrack.sampling_transition', true) <> 'create' then
...
begin
  ...
  select receipt.*
  from public.sampling_run run
  cross join lateral private.sampling_run_result(run.id) receipt
...
begin
  ...
  if not exists (select 1 from public.sampling_run where id = p_run_id and workspace_id = v_workspace) then
    raise exception using errcode = '42501', message = 'operation is not permitted';
  end if;
  return query
...
revoke all on function
  ...
  public.get_sampling_candidates(uuid)
  from public, anon, authenticated, service_role,
    palmtrack_audit_writer, palmtrack_recovery_executor;
